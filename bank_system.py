...
class datas:
    def __init__(self):
        self.activities = {"user_1": [0, []], "user_2": [ 0, []],
                           "user_3": [ 0, []]}
        self.members = {"user_1": "1234", "user_2": "4321", "user_3": "4422"}
    ...
    def continuous_structures(self, choice):
        if choice == "one":
            ...
            print "|-----------------------------------|"
            print "     Istanbul " + datetime.today().strftime('%d-%m-%y  %H:%M:%S')
            print "|-----------------------------------|"
    ...
    def user_verification(self, user_name, user_passwd):
        if self.members.has_key(user_name):
...
class deposit_and_withdraw:
    ...
    def deposit(self, name):
        money = raw_input("How much do you wish to deposite ? ")
        while int(money) < 1:
            print "Try Again..."
            money = raw_input("How much do you wish to deposite ? ")
    ...
    def withdraw(self, name):
        ...
        money = raw_input("How much do you wish to withdraw ? ")
...
class transfer_details:
    ...
    def transfer(self, name):
        ...
        for i in users:
            user_names.append(i)
        transferName = raw_input("For who do you wish to transfer to ?")
        if name in user_names and name != transferName:
            transferMoney = raw_input("How much would like to transfer ? ")
...
if __name__ == "__main__":
    data = datas()
    operation1 = deposit_and_withdraw(data)
    operation2 = transfer_details(data)
    program_end = False
                ...
                if data.user_verification(username, passwd):
                    ...
                    while keep_going_2:
                        ...
                        while choose != "1" and choose != "2" and choose != "3" and choose != "4" and choose != "5":
                            ...
                            choose = raw_input(">>>")
                        if choose == "1":
                            operation1.withdraw(username)
                        elif choose == "2":
                            operation1.deposit(username)
                        elif choose == "3":
                            operation2.transfer(username)
                        elif choose == "4":
                            operation2.details(username, passwd)
                        else:
                            keep_going_1 = False
                            keep_going_2 = False
                else:
